...
- **Continue**: プレイを継続（2048タイル同士の合体は不可）
- **Quit**: メニューに戻る
...
macOSでビルドする場合は、`Makefile` のmacOS用セクションを有効にしてください：

1. Linux用のセクションをコメントアウト
2. macOS用のセクション（63行目以降）のコメントを解除
3. `make` を実行

## ドキュメント

`docs/` ディレクトリに開発時の参考資料があります：

- `Home.md`: ncursesライブラリの基本と関数リファレンス
- `画面サイズ変更時の処理.md`: ウィンドウリサイズ処理のパターン
- `ncurses-とメモリリーク.md`: メモリリーク報告に関する議論
- `ncurses-aa-tiling.md`: ASCIIアートタイル描画
